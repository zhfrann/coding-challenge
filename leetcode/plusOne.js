 ...
 * @return {number[]}
 */
//! Won't work for big number like [6, 1, 4, 5, 3, 9, 0, 1, 9, 5, 1, 8, 6, 7, 0, 5, 5, 4, 3] using javascript
var plusOne = function (digits) {
    let result = Number(digits.join("")) + 1;
    result = result.toString().split("");
    return result.map((numString) => Number(numString));
};
...
//* Alternatives code for plusOne2
const plusOne3 = function (digits) {
    for (let i = digits.length - 1; i >= 0; i--) {
        if (digits[i] < 9) {
            digits[i]++;
            return digits;
